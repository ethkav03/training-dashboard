...
| 12 | Health Connect read + manual sync from the Android app (bounded historical read, mapper, `POST /integrations/health-connect/sync`) | Done (backend verified via curl; Android client written — `HealthConnectManager`/`Repository`/`Mapper`/`ViewModel`, then-standalone `SyncScreen` wiring, later absorbed into `SettingsScreen` in Sprint 15. Not compiled/run at the time; superseded by real Android Studio builds from Sprint 15 onward, see the note after this table) |
...
| 13 | Android `WorkManager` periodic incremental sync (Health Connect changes-token, token-expiry fallback) | Done (backend unaffected; Android client written — `SyncWorker`/`SyncScheduler`/`HealthConnectSyncState`. Not compiled/run at the time, same caveat as Sprint 12; superseded by real Android Studio builds from Sprint 15 onward). Web Settings' Health Connect row shipped earlier than planned, alongside Sprint 9's WHOOP row; on-device steps display deferred, see below. |
| 14 | Docs pass across all five `/docs` files for Sprints 8–13 + `.env.example` updates + Settings polish | Done |
| 15 | Android full-parity foundation: complete DTO/enum port, full Retrofit surface (every backend route), a repository per resource, a real bottom-nav shell (`ui/navigation/`) replacing the old two-screen if/else, shared UI primitives (`ui/components/`, `ui/cards/`), ported color palette, Health Connect UI relocated from the retired `SyncScreen` into the new `SettingsScreen` | Done — first sprint built and run for real in Android Studio (see the interim-fix row below for what that first real build turned up) |
...
| 20 | Android Timeline (week-paged, grouped-by-day, reached via a new "Full timeline" link on Today rather than its own bottom-nav tab -- matches web treating it as a `Link` off Today, not a `NAV_ITEMS` entry) + Insights (read-only). Every screen in the app is now real -- `PlaceholderScreens.kt` had nothing left to back and was deleted | Done |
| 21 | Android Settings' remaining sections (Units toggle, Energy baseline, Data controls -- export via Storage Access Framework picker + streamed `GET /users/me/export`, delete-account with an inline two-step confirm) + the Onboarding flow (single-form, not a wizard, matching `OnboardingPage.tsx`) + `MainActivity` gating (loading/login/onboarding/main-app, mirroring `ProtectedRoute.tsx`'s order) | Done |
| 22 | Parity/consistency pass, no new screens: `TodayScreen`'s Quick actions (and the empty-readiness-state "Log recovery" button) switched from tab navigation to the same inline `activeModal` bottom-sheet forms web's `TodayPage.tsx` uses, plus a previously-missing "Log recovery" Quick Action added. Root `README.md`'s Android section rewritten for full parity (was last accurate as of Sprint 14); stale Sprint 12/13/15 roadmap notes and a couple of drifted code comments corrected | Done |
