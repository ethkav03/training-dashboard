...
| POST | `/integrations/health-connect/sync` | Batch sync endpoint for the companion Android app — body is `{ weightRecords[], exerciseSessions[], sleepSessions[] }` (see `healthConnect.validation.ts`), called either from the app's manual "Sync now" (always a bounded 30-day read) or its `WorkManager` background job (incremental via Health Connect's own changes-token — see [architecture.md](./architecture.md#native-android-app)). Same manual-edit-precedence upsert path as WHOOP; sleep sessions run through `computeReadiness()` since Health Connect has no pre-computed score of its own. Response shape mirrors the WHOOP sync result (`status`, `syncedAt`, synced/skipped counts per record kind). |
